...
router.get('/learndot_enrollments', (req, res) => {
...
async function get_learndot_events_enrollments() {
	await deleteenrollments();
	// create the query and set to variable
		...
		for(i=1; i <= Object.keys(rows).length; i++){
			if(rows[i] != undefined){
				for(var i in rows) {
					...
					//console.log(ld_event_id + "|" + ld_start_time + "|" + ld_email)
