	...
	(async () => {
        try {
          ...
          for(i=1; i <= Object.keys(rows).length; i++){
            if(rows[i] != undefined){
                for(var i in rows) {
                    console.log(JSON.stringify(rows[i].event_id), JSON.stringify(rows[i].start_time), JSON.stringify(rows[i].email))
                }
            }
        }
          // console.log(rows);
...
function deleteEventsAndEnrollments() {
	return new Promise((resolve, reject) => {
		...
		// execute the query
		database.pool.query(sql_delete_events_and_enrollments, (error, elements) => {
			if(error){
                return reject(error);
            }
